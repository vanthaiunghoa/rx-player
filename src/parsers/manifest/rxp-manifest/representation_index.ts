  ...
  return {
    ...
    getInitSegment() : ISegment|null {
      ...
      return {
        ...
        mediaURL: null,
        privateInfos: {
          rxpSegment: { load: index.init.load },
        },
      };
    },
    ...
    getSegments(up : number, duration : number) : ISegment[] {
      ...
      const wantedSegments : IRXPIndexSegment[] = [];
      ...
      return wantedSegments
        .map(wantedSegment => {
          return {
            ...
            mediaURL: null,
            privateInfos: {
              rxpSegment: { load: wantedSegment.load },
            },
          };
        });
    },
    ...
    _update() : void {
      if (__DEV__) {
        log.warn("Tried to update a rxp-manifest RepresentationIndex");
      }
    },

    _addSegments() : void {
      if (__DEV__) {
        log.warn("Tried to add Segments to a rxp-manifest RepresentationIndex");
      }
    },
  };
}
